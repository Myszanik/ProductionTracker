...
app = Flask(__name__)
app.secret_key = "CHANGE_ME_TO_A_LONG_RANDOM_STRING"
DB_PATH = "orders.db"
...
def advance_lorry(slot: int):
    ...
    l2 = int(get_setting(KEY_LORRY2, "2"))
    nx = int(get_setting(KEY_NEXT,   str(max(l1, l2) + 1)))
    if slot == 1: set_setting(KEY_LORRY1, str(nx))
    else:         set_setting(KEY_LORRY2, str(nx))
...
def login():
    ...
    if request.method == "POST":
        ...
        if row and check_password_hash(row[2], password):
            session["user_id"]=row[0]; session["username"]=row[1]; session["role"]=row[3]; session["area"]=row[4]
...
def preparing_station():
    ...
    if request.method == "POST":
        ...
        if dup:
            ...
            return render_template("confirm.html",
                ...
                confirm_name=None, cancel_url=url_for("preparing_station"), post_url=None, hidden_fields={}
            )
        # capacity
        ...
        if count_pending >= QUEUE_CAPACITY_PREP:
            ...
            for cnc in CNC_STATIONS:
                ...
                (cnt,) = c.fetchone()
                if cnt < QUEUE_CAPACITY_PREP: available.append(cnc.upper())
            if not available:
                conn.close()
                return render_template("confirm.html",
        ...
        if not request.form.get("confirm"):
            ...
            return render_template("confirm.html",
                ...
                confirm_name="confirm", cancel_url=url_for("preparing_station"),
                post_url=url_for("preparing_station"),
                hidden_fields={"order_number": order_no, "target_cnc": target_cnc})
        c.execute("INSERT INTO orders (order_number, status, current_station) VALUES (?, 'Pending', ?)",
                  (order_no, target_cnc))
        oid = c.lastrowid
        conn.commit(); conn.close()
        # history
    ...
    for cnc in CNC_STATIONS:
        c.execute("""
            ...
            FROM orders WHERE current_station=? AND status='Pending'
...
def cnc_station():
    area = (session.get("area") or "").lower()
    if area not in CNC_STATIONS: return ("Forbidden: not a CNC station", 403)
    conn = get_db(); c = conn.cursor()
    c.execute("""
    ...
    c.execute("""
        ...
        FROM orders WHERE current_station=? AND status='In progress'
    ...
    c.execute("""
        ...
        FROM orders WHERE current_station=? AND status='Done'
        ORDER BY finished_at ASC NULLS LAST, queued_at ASC
    """, (area,))
    done = c.fetchall()

    if request.method == "POST":
        action = request.form.get("action"); oid = request.form.get("order_id")
        if action == "start":
            if inprog:
                conn.close()
                return render_template("confirm.html", title="Cannot start",
                    message="You already have a job In progress. Finish it before starting another.",
                    confirm_name=None, cancel_url=url_for("cnc_station"), post_url=None, hidden_fields={})
            first_id = pending[0][0] if pending else None
            if str(oid) != str(first_id):
                conn.close()
                return render_template("confirm.html", title="Cannot start",
                    message="You can only start the first Pending job.", confirm_name=None,
                    cancel_url=url_for("cnc_station"), post_url=None, hidden_fields={})
            if not request.form.get("confirm"):
                conn.close()
                return render_template("confirm.html", title="Confirm start",
                    message=f"Start this job on {area.upper()}?", confirm_name="confirm",
                    cancel_url=url_for("cnc_station"), post_url=url_for("cnc_station"),
                    hidden_fields={"action":"start","order_id":oid})
            d = get_db(); dc = d.cursor()
            dc.execute("UPDATE orders SET status='In progress', started_at=CURRENT_TIMESTAMP WHERE id=? AND current_station=? AND status='Pending'", (oid, area))
        ...
        if action == "finish":
            if not request.form.get("confirm"):
                conn.close()
                return render_template("confirm.html", title="Confirm finish",
                    message="Mark this job Finished on CNC?", confirm_name="confirm",
                    cancel_url=url_for("cnc_station"), post_url=url_for("cnc_station"),
                    hidden_fields={"action":"finish","order_id":oid})
            d = get_db(); dc = d.cursor()
            dc.execute("UPDATE orders SET status='Done', finished_at=CURRENT_TIMESTAMP WHERE id=? AND current_station=? AND status='In progress'", (oid, area))
            d.commit(); d.close(); conn.close()
            log_many([(int(oid), area, "done"), (int(oid), TRAMMING1_STATION, "pending")])
            return redirect(url_for("cnc_station"))
    conn.close()
    return render_template("station_cnc.html", station=area.upper(), pending=pending, inprog=inprog, done=done)
...
def tramming1_station():
    ...
    if request.method == "POST" and request.form.get("action") == "assign":
        order_id = request.form.get("order_id"); src_cnc = request.form.get("src_cnc"); tgt_edge = request.form.get("tgt_edge")
        first_id = cnc_done.get(src_cnc, [None])
        first_id = first_id[0][0] if first_id else None
        if str(order_id) != str(first_id):
            conn.close()
            return render_template("confirm.html", title="Cannot assign",
                message="Only the first Finished job in each CNC lane can be assigned.",
                confirm_name=None, cancel_url=url_for("tramming1_station"), post_url=None, hidden_fields={})
        if not tgt_edge:
            conn.close()
            return render_template("confirm.html", title="Choose Edge Bander",
                message="Please select an Edge Bander.", confirm_name=None,
                cancel_url=url_for("tramming1_station"), post_url=None, hidden_fields={})
        ...
        if count_pending >= cap:
            ...
            for ed in EDGE_STATIONS:
                ...
                (cnt,) = c.fetchone()
                if cnt < EDGE_CAPACITY[ed]: available.append(ed.upper())
            msg = f"{tgt_edge.upper()} is full ({count_pending}/{cap})."
            if available: msg += " Available: " + ", ".join(available)
            conn.close()
            return render_template("confirm.html", title="Capacity full",
                message=msg, confirm_name=None, cancel_url=url_for("tramming1_station"),
                post_url=None, hidden_fields={})
        if not request.form.get("confirm"):
            conn.close()
            return render_template("confirm.html", title="Confirm assignment",
                message=f"Assign order to {tgt_edge.upper()}?",
                confirm_name="confirm", cancel_url=url_for("tramming1_station"),
    ...
    conn.close()
    return render_template("station_tramming1.html", cnc_done=cnc_done, edge_pending=edge_pending, capacities=EDGE_CAPACITY)
...
def edge_station():
    area = (session.get("area") or "").lower()
    if area not in EDGE_STATIONS: return ("Forbidden: not an Edge station", 403)
    conn = get_db(); c = conn.cursor()
    c.execute("""
        SELECT id, order_number, datetime(queued_at,'localtime')
        FROM orders WHERE current_station=? AND status='Pending'
    ...
    if request.method == "POST":
        ...
        if action == "start":
            if inprog:
                ...
                return render_template("confirm.html", title="Cannot start",
                    message="You already have a job In progress. Finish it before starting another.",
                    confirm_name=None, cancel_url=url_for("edge_station"), post_url=None, hidden_fields={})
            first_id = pending[0][0] if pending else None
            if str(oid) != str(first_id):
                conn.close()
                return render_template("confirm.html", title="Cannot start",
                    message="You can only start the first Pending job.", confirm_name=None,
                    cancel_url=url_for("edge_station"), post_url=None, hidden_fields={})
            if not request.form.get("confirm"):
                conn.close()
                return render_template("confirm.html", title="Confirm start",
                    message=f"Start this job on {area.upper()}?", confirm_name="confirm",
                    cancel_url=url_for("edge_station"), post_url=url_for("edge_station"),
                    hidden_fields={"action":"start","order_id":oid})
            d = get_db(); dc = d.cursor()
            dc.execute("UPDATE orders SET status='In progress', started_at=CURRENT_TIMESTAMP WHERE id=? AND current_station=? AND status='Pending'", (oid, area))
            d.commit(); d.close(); conn.close()
        ...
        if action == "finish":
            if not request.form.get("confirm"):
                conn.close()
                return render_template("confirm.html", title="Confirm finish",
                    message="Mark this job Finished on Edge?", confirm_name="confirm",
                    cancel_url=url_for("edge_station"), post_url=url_for("edge_station"),
                    hidden_fields={"action":"finish","order_id":oid})
            d = get_db(); dc = d.cursor()
            dc.execute("UPDATE orders SET status='Done', finished_at=CURRENT_TIMESTAMP WHERE id=? AND current_station=? AND status='In progress'", (oid, area))
            ...
            return redirect(url_for("edge_station"))
    conn.close()
    return render_template("station_edge.html", station=area.upper(), cap=EDGE_CAPACITY.get(area,0), pending=pending, inprog=inprog, done=done)
...
def tramming2_station():
    ...
    c.execute("""
        ...
        FROM orders
        WHERE current_station='wrapping' AND wrap_slot IS NOT NULL AND status IN ('Pending','In progress')
    ...
    if request.method == "POST" and request.form.get("action") == "assign_wrap":
        if all_full:
            ...
            return render_template("confirm.html",
                ...
                confirm_name=None, cancel_url=url_for("tramming2_station"), post_url=None, hidden_fields={})
        order_id = request.form.get("order_id"); src_edge = request.form.get("src_edge")
        try: slot = int(request.form.get("wrap_slot") or 0)
        except: slot = 0
        if slot not in WRAP_SLOTS:
            conn.close()
            return render_template("confirm.html", title="Choose slot",
                message="Please choose Wrapping slot 1, 2, or 3.", confirm_name=None,
                cancel_url=url_for("tramming2_station"), post_url=None, hidden_fields={})
        first_id = edge_done.get(src_edge, [None])
        first_id = first_id[0][0] if first_id else None
        if str(order_id) != str(first_id):
            conn.close()
            return render_template("confirm.html", title="Cannot move",
        ...
        if slot in wrap_occ:
            ...
            if free: msg += " Available slots: " + ", ".join(free)
            conn.close()
            return render_template("confirm.html", title="Slot occupied",
                message=msg, confirm_name=None, cancel_url=url_for("tramming2_station"),
                post_url=None, hidden_fields={})
        if not request.form.get("confirm"):
            conn.close()
            return render_template("confirm.html", title="Confirm move",
                message=f"Move this job to WRAPPING, slot {slot}?",
                confirm_name="confirm", cancel_url=url_for("tramming2_station"),
                post_url=url_for("tramming2_station"),
                hidden_fields={"action":"assign_wrap","order_id":order_id,"src_edge":src_edge,"wrap_slot":slot})
        d = get_db(); dc = d.cursor()
        dc.execute("""
            UPDATE orders
            SET current_station='wrapping', status='Pending', queued_at=CURRENT_TIMESTAMP, wrap_slot=?
            WHERE id=? AND current_station=? AND status='Done'
    ...
    conn.close()
    return render_template("station_tramming2.html", edge_done=edge_done, wrap_occ=wrap_occ, wrap_slots=WRAP_SLOTS)
...
def wrapping_station():
    ...
    c.execute("""
        ...
        FROM orders WHERE current_station='wrapping' AND status='Done'
    ...
    if request.method == "POST":
        ...
        if action == "start":
            if not request.form.get("confirm"):
                conn.close()
                return render_template("confirm.html", title="Confirm start",
                    message="Start this job in WRAPPING?", confirm_name="confirm",
                    cancel_url=url_for("wrapping_station"), post_url=url_for("wrapping_station"),
                    hidden_fields={"action":"start","order_id":oid})
            d = get_db(); dc = d.cursor()
            dc.execute("UPDATE orders SET status='In progress', started_at=CURRENT_TIMESTAMP WHERE id=? AND current_station='wrapping' AND status='Pending'", (oid,))
            d.commit(); d.close(); conn.close()
        ...
        if action == "finish":
            if not request.form.get("confirm"):
                conn.close()
                return render_template("confirm.html", title="Confirm finish",
                    message="Mark this job Finished in WRAPPING?", confirm_name="confirm",
                    cancel_url=url_for("wrapping_station"), post_url=url_for("wrapping_station"),
                    hidden_fields={"action":"finish","order_id":oid})
            d = get_db(); dc = d.cursor()
            dc.execute("UPDATE orders SET status='Done', finished_at=CURRENT_TIMESTAMP, wrap_slot=NULL WHERE id=? AND current_station='wrapping' AND status='In progress'", (oid,))
            d.commit(); d.close(); conn.close()
            log_status(int(oid), WRAPPING_STATION, "done")
            return redirect(url_for("wrapping_station"))
    conn.close()
    return render_template("station_wrapping.html", wrap_occ=wrap_occ, wrap_slots=WRAP_SLOTS, done=done)
    ...
    if request.method == "POST":
        ...
        if action == "start_batch":
            ...
            if lorry_sel not in ("1","2"):
                conn.close()
                return render_template("confirm.html", title="Choose lorry",
                    message="Please select Lorry 1 or Lorry 2.", confirm_name=None,
                    cancel_url=url_for("loading_station"), post_url=None, hidden_fields={})
            if not ids or len(ids) > 2:
                conn.close()
                return render_template("confirm.html", title="Selection error",
                    message="Select 1 or 2 jobs to start loading.", confirm_name=None,
                    cancel_url=url_for("loading_station"), post_url=None, hidden_fields={})
            label = lorry1_label if lorry_sel=="1" else lorry2_label
            if not request.form.get("confirm"):
                conn.close()
                return render_template("confirm.html", title="Confirm start loading",
                    message=f"Start loading {len(ids)} job(s) to {label}?",
                    confirm_name="confirm", cancel_url=url_for("loading_station"),
                    post_url=url_for("loading_station"),
                    hidden_fields={"action":"start_batch","lorry":lorry_sel,"order_ids":",".join(ids)})
            d = get_db(); dc = d.cursor()
            batch = str(uuid.uuid4()) if len(ids) > 1 else None
            for oid in ids:
                dc.execute("""
                    UPDATE orders
                    SET current_station='loading', status='In progress', started_at=CURRENT_TIMESTAMP, lorry=?, batch_id=?
        ...
        if action == "finish":
            ...
            d = get_db(); dc = d.cursor()
            dc.execute("SELECT batch_id FROM orders WHERE id=? AND current_station='loading' AND status='In progress'", (oid,))
            row = dc.fetchone()
            if not request.form.get("confirm"):
                d.close()
                return render_template("confirm.html", title="Confirm finish",
                    ...
                    hidden_fields={"action":"finish","order_id":oid})
            if row and row[0]:
                dc.execute("UPDATE orders SET status='Done', finished_at=CURRENT_TIMESTAMP WHERE batch_id=? AND current_station='loading' AND status='In progress'", (row[0],))
                dc.execute("SELECT id FROM orders WHERE batch_id=? AND current_station='loading' AND status='Done'", (row[0],))
                ids = [r[0] for r in dc.fetchall()]
                d.commit(); d.close()
                log_many([(int(i), LOADING_STATION, "done") for i in ids])
            else:
                dc.execute("UPDATE orders SET status='Done', finished_at=CURRENT_TIMESTAMP WHERE id=? AND current_station='loading' AND status='In progress'", (oid,))
                d.commit(); d.close()
        ...
        if action in ("complete_lorry1","complete_lorry2"):
            ...
            if fin_count < LORRY_CAPACITY or inprog_count > 0:
                conn.close()
                return render_template("confirm.html", title="Cannot complete",
                    message="Lorry is not fully loaded yet.",
                    confirm_name=None, cancel_url=url_for("loading_station"), post_url=None, hidden_fields={})
            advance_lorry(1 if left_side else 2)
            return redirect(url_for("loading_station"))

    conn.close()
    return render_template("station_loading.html",
                           ready=ready,
                           inprog_l1=inprog_l1, inprog_l2=inprog_l2,
                           fin_l1=fin_l1, fin_l2=fin_l2,
                           lorry1_label=lorry1_label, lorry2_label=lorry2_label,
                           count_l1=count_l1, count_l2=count_l2,
...
def manager_view():
    ...
    c.execute("""
        ...
        JOIN latest l
          ON l.order_id = h.order_id AND l.station = h.station AND l.latest_ts = h.changed_at
    """)
    ...
    # Also pull a raw lookup of the orders table (incl. lorry label)
    cur = get_db().cursor()
    cur.execute("""
    ...
    raw = {
        r[0]: {
            ...
            "lr":  r[6]  # lorry label, e.g., "Lorry 7"
    ...
    for oid, station_map in per_order_raw.items():
        per_order[oid] = {}
        # simple stations
        for st in [PREPARING_STATION, TRAMMING1_STATION, TRAMMING2_STATION, WRAPPING_STATION, LOADING_STATION]:
        ...
        # CNC consolidated
        cnc_cells = [(m, station_map[m]["status"], station_map[m]["ts"]) for m in AREA_GROUPS["cnc"] if m in station_map]
        latest_cnc = pick_latest(cnc_cells)
        if latest_cnc:
            mname, st_status, st_ts = latest_cnc
            per_order[oid]["cnc"] = {"status": st_status, "ts": st_ts, "machine": mname.upper()}
        # EDGE consolidated
        edge_cells = [(m, station_map[m]["status"], station_map[m]["ts"]) for m in AREA_GROUPS["edge"] if m in station_map]
        latest_edge = pick_latest(edge_cells)
        if latest_edge:
            mname, st_status, st_ts = latest_edge
            per_order[oid]["edge"] = {"status": st_status, "ts": st_ts, "machine": mname.upper()}
        ...
        if lr and LOADING_STATION in per_order[oid]:
            per_order[oid][LOADING_STATION]["machine"] = lr.upper()  # shows "on LORRY 12"

    # Fallback to orders table for completion detection (and Loading lorry label)
    ...
    c.execute("SELECT COUNT(*) FROM orders WHERE current_station='loading' AND status='Done'")
    (orders_fully_done,) = c.fetchone()

    per_area_done = {}
    for st in [PREPARING_STATION, *CNC_STATIONS, TRAMMING1_STATION, *EDGE_STATIONS, TRAMMING2_STATION, WRAPPING_STATION, LOADING_STATION]:
        c.execute("SELECT COUNT(*) FROM order_history WHERE station=? AND status='done'", (st,))
        per_area_done[st] = (c.fetchone() or [0])[0]

    def lorry_progress(label):
        c2 = get_db().cursor()
        c2.execute("SELECT COUNT(*) FROM orders WHERE current_station='loading' AND status='Done' AND lorry=?", (label,))
    ...
    def human_group(station_code):
        ...
        if s in AREA_GROUPS.get("cnc", []):  return "CNC", f" ({s.upper()})"
        if s in AREA_GROUPS.get("edge", []): return "EDGE", f" ({s.upper()})"
    ...
    if search_mode:
        ...
        if row:
            search_found = True
            search_oid, search_onum, cur_st, cur_status, qts, sts, fts, lorry_lbl = row
            ...
            if cur_st == LOADING_STATION and cur_status == "Done":
                ...
                when = (sts if cur_status == "In progress" else (qts if cur_status == "Pending" else fts)) or "—"
                search_current_line = f"Current location: LOADING{lorry_hint} ({cur_status.lower()}) since {when}"
            elif cur_status == "In progress":
                when = sts or "—"
                search_current_line = f"Current location: {label}{machine_hint} (in progress) since {when}"
            elif cur_status == "Pending":
                when = qts or "—"
                search_current_line = f"Current location: {label}{machine_hint} (pending) since {when}"
            elif cur_status == "Done":
                when = fts or "—"
                search_current_line = f"Current location: {label}{machine_hint} (done) at {when}"
            else:
                cells = per_order.get(search_oid, {})
                latest = None
                for st_code, cell in cells.items():
                    if (not latest) or (cell["ts"] > latest["ts"]):
                        latest = {"st": st_code, "status": cell["status"], "ts": cell["ts"]}
                if latest:
                    lab, mh = human_group(latest["st"])
                    search_current_line = f"Current location: {lab}{mh} ({latest['status'].replace('_',' ')}) since {latest['ts']}"
                else:
            ...
            if cur_st:
                key = "cnc" if cur_st in AREA_GROUPS["cnc"] else ("edge" if cur_st in AREA_GROUPS["edge"] else cur_st)
                ts_guess = qts if cur_status == "Pending" else (sts if cur_status == "In progress" else (fts or "—"))
                per_order[search_oid][key] = {
                    "status": cur_status.replace(" ", "_").lower(),
                    "ts": ts_guess,
                    "machine":
                        (cur_st.upper() if key in ("cnc","edge") else
                         (lorry_lbl or "").upper() if key == LOADING_STATION else
                         None)
                }
    ...
    return render_template("manager.html",
                           ...
                           lorry1_label=l1_label, lorry2_label=l2_label,
                           l1_done=l1_done, l2_done=l2_done,
                           l1_pct=l1_pct, l2_pct=l2_pct,
                           ...
                           q=q, search_mode=search_mode, search_found=search_found,
                           search_oid=search_oid, search_onum=search_onum,
